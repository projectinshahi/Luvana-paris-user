...
export default function   Navbar() {
  ...
  if (!mounted || !ready) {
    return (
      <>
        {/* Top Offer Bar */}
        <div className="fixed top-0 left-0 right-0 bg-[#0A0A0A] text-white text-center text-xs sm:text-sm py-1.5 sm:py-2 z-50">
          <span className="block text-[10px] sm:text-xs">Buy for ₹999 and enjoy a cool 25% OFF!</span>
        </div>

        {/* Mobile Top Navbar - Menu | Logo | Settings */}
        <nav className="sm:hidden fixed top-[24px] left-0 right-0 bg-[#0D0D0D] text-white px-4 py-3 z-40 border-b border-gray-800 flex items-center justify-between">
        ...
        <nav className="hidden sm:flex fixed top-[24px] sm:top-[36px] left-0 right-0 bg-[#0D0D0D] text-white px-2 sm:px-4 md:px-10 py-1.5 sm:py-2 md:py-3 z-40 no-rtl-transform">
          ...
          <div className="flex items-center gap-1 sm:gap-3 md:gap-4 flex-no-reverse shrink-0" style={{ order: 1 }}>
            <button className="p-1 sm:p-1.5 hover:bg-gray-700 rounded transition-colors">
              <Menu size={16} className="sm:size-[18px]" />
            </button>
          ...
          <div className="flex items-center gap-0.5 sm:gap-1.5 md:gap-2 flex-no-reverse shrink-0" style={{ order: 3 }}>
            ...
            <button className="p-1 sm:p-1.5 hover:bg-gray-700 rounded transition-colors ml-0.5">
              <Settings size={16} className="text-white sm:size-[18px]" />
            </button>
          </div>
        </div>
        </nav>
  ...
  return (
    <>
      ...
      {/* Mobile Top Navbar - Menu | Logo | Settings */}
      <nav className="sm:hidden fixed top-[24px] left-0 right-0 bg-[#0D0D0D] text-white px-4 py-3 z-40 border-b border-gray-800 flex items-center justify-between" suppressHydrationWarning>
        <button 
      ...
      <nav className="hidden sm:flex fixed top-[24px] sm:top-[36px] left-0 right-0 bg-[#0D0D0D] text-white px-2 sm:px-4 md:px-10 py-1.5 sm:py-2 md:py-3 z-40 no-rtl-transform" suppressHydrationWarning>
        <div className="flex items-center justify-between navbar-flex w-full">
          {/* Left Section - Logo */}
          <div className="flex items-center gap-1 sm:gap-3 md:gap-4 flex-no-reverse flex-shrink-0" style={{ order: 1 }}>
            <button 
          ...
          {/* Right Section - Icons */}
          <div className="flex items-center gap-0.5 sm:gap-1.5 md:gap-2 flex-no-reverse flex-shrink-0" style={{ order: 3 }}>
            <button className="p-1 sm:p-1.5 hover:bg-gray-700 rounded transition-colors">
              <Heart
            ...
            <button className="p-1 sm:p-1.5 hover:bg-gray-700 rounded transition-colors">
              <User className="cursor-pointer hover:text-gray-300 transition-colors" size={14} />
            </button>
            <button className="p-1 sm:p-1.5 hover:bg-gray-700 rounded transition-colors">
      ...
      <nav className="sm:hidden fixed bottom-0 left-0 right-0 bg-[#0D0D0D] text-white px-0 py-2 z-40 border-t border-gray-800 flex items-center justify-around" suppressHydrationWarning>
        <button className="p-3 rounded-lg hover:bg-gray-800 transition-colors text-yellow-400 bg-gray-800">
          <Home size={20} />
        </button>
        <button className="p-3 rounded-lg hover:bg-gray-800 transition-colors hover:text-red-500">
