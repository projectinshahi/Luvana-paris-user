      ...
      <section className="w-full bg-black text-white py-8 sm:py-12 lg:py-16 overflow-hidden">
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative w-full overflow-hidden pause-on-hover">
            <div
              className={`flex w-max gap-3 sm:gap-4 md:gap-6 lg:gap-8 ${
                isRTL ? "bestseller-scroll-reverse" : "bestseller-scroll"
              }`}
            >
              {[...products, ...products].map((product, index) => (
                <div
                  key={`${product.id}-${index}`}
                  className="shrink-0 w-40 sm:w-50 md:w-55 lg:w-65 bg-[#111] rounded-lg overflow-hidden hover:scale-[1.02] transition-transform duration-300"
                >
                  {/* IMAGE */}
                  <div className="group relative w-full aspect-3/4 bg-linear-to-b from-[#E3C6A8] to-[#5F4D2B] p-1">
                    <div className="relative w-full h-full bg-[#D9D9D9] overflow-hidden rounded-sm">
                      ...
                      {/* WISHLIST */}
                      <button
                        className={`absolute top-2 ${isRTL ? "left-2" : "right-2"} z-10 w-8 h-8 rounded-full bg-linear-to-b from-[#F7E7B4] via-[#D4AF37] to-[#8C6B1F] flex items-center justify-center opacity-0 group-hover:opacity-100 transition`}
                      >
                      ...
                      {/* ADD TO CART */}
                      <button
                        className={`absolute bottom-2 left-1/2 -translate-x-1/2 w-[90%] h-9 rounded-lg bg-linear-to-b from-[#F7E7B4] via-[#D4AF37] to-[#8C6B1F] text-black text-xs opacity-0 group-hover:opacity-100 transition ${
                          isRTL ? "font-arabic" : ""
                        }`}
                      >
                        {t("products.addToCart")}
                  ...
                  {/* TEXT */}
                  <div className="p-3">
                    <h3 className={`text-xs mb-1 ${isRTL ? "text-right font-arabic" : "text-left"}`}>
                      {t(product.nameKey)}
                    </h3>
                    <p className={`text-[#C9A24D] text-xs ${isRTL ? "text-right" : "text-left"}`}>
                      {product.price}
                    </p>
                  </div>
                </div>
